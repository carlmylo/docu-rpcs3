...
* Baterías de Rock Band mandan mas de un botón a la vez. Por ejemplo, el Parche Rojo manda "`B`" y "`Palanca izquierda presionada`" (que para el emulador es "`Circle (Circulo)`" y "`L3`".) Toma en cuenta que esto se requiere para que funcionen los platillos correctamente.
	* Recomendamos que uses un mando de Xbox para mapear todo para después cambiar el XInput Device a tu batería de Rock Band para Xbox 360.
